...
class JsonResponse extends Response
{
    ...
    protected function update()
    {
        if (null !== $this->callback) {
            // Not using application/js for compatibility reasons with older browsers.
            $this->headers->set('Content-Type', 'text/js');

            return $this->setContent(sprintf('/**/%s(%s);', $this->callback, $this->data));
        }

        // Only set the header when there is none or when it equals 'text/js' (from a previous update with callback)
        // in order to not overwrite a custom definition.
        if (!$this->headers->has('Content-Type') || 'text/js' === $this->headers->get('Content-Type')) {
